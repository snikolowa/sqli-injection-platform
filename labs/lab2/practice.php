...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // intentionally vulnerable query (boolean-based)
    $sql = "SELECT id FROM products WHERE name = '$input' LIMIT 1";
    $result = mysqli_query($conn, $sql);

    // Success condition in this lab (based on existing logic in your original file)
    // We'll keep the same concept: if the query returns at least 1 row -> solved.
    if ($result && mysqli_num_rows($result) > 0) {
        $message = "🎉 Успешно! Успя да манипулираш boolean логиката.";
        $completedNow = true;
    } else {
...
<div class="card shadow-sm">
  <div class="card-body">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start gap-2">
      <div>
        <h1 class="h4 fw-bold mb-1">Модул 2: Practice – Boolean-based Blind SQLi</h1>
        <p class="text-secondary mb-0">
          Цел: да накараш заявката да връща резултат чрез boolean условие.
        </p>
      </div>
    ...
    <form method="post" class="row g-3 mt-2" autocomplete="off">
      <div class="col-12">
        ...
        <input type="text" name="q" class="form-control" required placeholder="product name">
      </div>
